...
class Estimation():
    ...
    Therefore, the update step compares the propagated states using the last parameter estimates
    and the observed states to obtain the latest parameter estimates.
    ...
    def __init__(self, prop, use_vels, sim):
        ...
        self.prop_obj = prop
        self.reset_performed = False # track if and when the estimator was reset
    ...
    def initialize_param_est(self):
        ...
        if self.DoF == 3:
            if self.sim == "true":
                ...
                self.P = param_est_config.P_3DOF_simplified_hw
                self.Q = param_est_config.Q_3DOF_simplified_hw
                self.R = param_est_config.R_3DOF_hw
            ...
            if self.sim == "true":
                ...
                self.P = param_est_config.P_6DOF_simplified_hw
                self.Q = param_est_config.Q_6DOF_simplified_hw
    ...
    def state_propagation(self, dt, control_ip):
        ...
        if self.n_meas == self.DoF: # No linear velocity measurements,
            ...
            #print(self.params_est[0:self.n_params])
    ...
    def update_step(self, dt, state_meas):
        ...
        self.states_prop = z_tilde # should the self.states_prop be initialized every time?
        if Quiet==0:
