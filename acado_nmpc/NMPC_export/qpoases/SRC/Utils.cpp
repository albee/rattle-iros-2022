...
returnValue myPrintf(const char* s) {
  ...
  if (outputfile == 0) return THROWERROR(RET_NO_GLOBAL_MESSAGE_OUTPUTFILE);
...
returnValue printCopyrightNotice() {
  return myPrintf(
    "\nqpOASES -- An Implementation of the Online Active Set Strategy.\nCopyright (C) 2007-2008 by Hans Joachim "
    "Ferreau et al. All rights reserved.\n\nqpOASES is distributed under the terms of the \nGNU Lesser General Public "
    "License 2.1 in the hope that it will be \nuseful, but WITHOUT ANY WARRANTY; without even the implied warranty "
...
returnValue readFromFile(real_t* data, int nrow, int ncol, const char* datafilename) {
  ...
  if ((datafile = fopen(datafilename, "r")) == 0) {
    ...
    return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_OPEN_FILE, errstr, __FUNCTION__, __FILE__, __LINE__,
                                                 VS_VISIBLE);
  }
  ...
  for (i = 0; i < nrow; ++i) {
    for (j = 0; j < ncol; ++j) {
      if (fscanf(datafile, "%f ", &float_data) == 0) {
        ...
        char errstr[80];
        sprintf(errstr, "(%s)", datafilename);
        return getGlobalMessageHandler()->throwError(RET_UNABLE_TO_READ_FILE, errstr, __FUNCTION__, __FILE__, __LINE__,
                                                     VS_VISIBLE);
      }
...
returnValue writeIntoFile(const real_t* const data, int nrow, int ncol, const char* datafilename, BooleanType append) {
  ...
  for (i = 0; i < nrow; ++i) {
    for (j = 0; j < ncol; ++j) fprintf(datafile, "%.16e ", data[i * ncol + j]);
 ...
 *	w r i t e I n t o F i l e
 */
returnValue writeIntoFile(const int* const data, int n, const char* datafilename, BooleanType append) {
  int i;
  ...
  /* 2) Write data into file. */
  for (i = 0; i < n; ++i) fprintf(datafile, "%d\n", data[i]);
