 ...
 */
CyclingManager::CyclingManager() : nV(0), nC(0) { cyclingDetected = BT_FALSE; }

/*
 *	C y c l i n g M a n a g e r
 */
CyclingManager::CyclingManager(const CyclingManager& rhs)
    : nV(rhs.nV), nC(rhs.nC), cyclingDetected(rhs.cyclingDetected) {
  int i;

  for (i = 0; i < nV + nC; ++i) status[i] = rhs.status[i];
}
 ...
 */
returnValue CyclingManager::setCyclingStatus(int number, BooleanType isBound, CyclingStatus _status) {
  if (isBound == BT_TRUE) {
...
CyclingStatus CyclingManager::getCyclingStatus(int number, BooleanType isBound) const {
  if (isBound == BT_TRUE) {
    ...
    if ((number >= 0) && (number < nV)) return status[number];
  } else {
    /* Return cycling status of a constraint. */
    if ((number >= 0) && (number < nC)) return status[nV + number];
  }
...
returnValue CyclingManager::clearCyclingData() {
  ...
  for (i = 0; i < nV + nC; ++i) status[i] = CYC_NOT_INVOLVED;
