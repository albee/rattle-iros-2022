 ...
 *	B o u n d s
 */
Bounds::Bounds(const Bounds& rhs) : SubjectTo(rhs), nV(rhs.nV), nFV(rhs.nFV), nBV(rhs.nBV), nUV(rhs.nUV) {
  free = rhs.free;
  fixed = rhs.fixed;
}
...
returnValue Bounds::setupBound(int _number, SubjectToStatus _status) {
  /* consistency check */
  if ((_number < 0) || (_number >= getNV())) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);
...
returnValue Bounds::setupAllFree() {
  ...
  for (i = 0; i < nV; ++i) {
    if (getType(i) == ST_UNBOUNDED) {
      if (setupBound(i, ST_INACTIVE) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_BOUND_FAILED);
    }
  }
...
returnValue Bounds::moveFixedToFree(int _number) {
  ...
  if (this->removeIndex(this->getFixed(), _number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Bounds::moveFreeToFixed(int _number, SubjectToStatus _status) {
  ...
  if (this->removeIndex(this->getFree(), _number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
