...
QProblem::QProblem(const QProblem& rhs) : QProblemB(rhs) {
  ...
  for (i = 0; i < _nC; ++i)
    for (j = 0; j < _nV; ++j) A[i * NVMAX + j] = rhs.A[i * NVMAX + j];

  for (i = 0; i < _nC; ++i) lbA[i] = rhs.lbA[i];

  for (i = 0; i < _nC; ++i) ubA[i] = rhs.ubA[i];

  constraints = rhs.constraints;

  for (i = 0; i < (_nV + _nC); ++i) y[i] = rhs.y[i];

  sizeT = rhs.sizeT;

  for (i = 0; i < sizeT; ++i)
    for (j = 0; j < sizeT; ++j) T[i * NVMAX + j] = rhs.T[i * NVMAX + j];

  for (i = 0; i < _nV; ++i)
    for (j = 0; j < _nV; ++j) Q[i * NVMAX + j] = rhs.Q[i * NVMAX + j];

  for (i = 0; i < _nC; ++i) Ax[i] = rhs.Ax[i];
...
returnValue QProblem::reset() {
  ...
  if (QProblemB::reset() != SUCCESSFUL_RETURN) return THROWERROR(RET_RESET_FAILED);
  ...
  for (i = 0; i < sizeT; ++i)
    for (j = 0; j < sizeT; ++j) T[i * NVMAX + j] = 0.0;

  for (i = 0; i < nV; ++i)
    for (j = 0; j < nV; ++j) Q[i * NVMAX + j] = 0.0;

  /* 4) Reset cycling manager. */
  if (cyclingManager.clearCyclingData() != SUCCESSFUL_RETURN) return THROWERROR(RET_RESET_FAILED);
...
returnValue QProblem::init(const real_t* const _H, const real_t* const _g, const real_t* const _A,
                           const real_t* const _lb, const real_t* const _ub, const real_t* const _lbA,
                           const real_t* const _ubA, int& nWSR, const real_t* const yOpt, real_t* const cputime) {
  /* 1) Setup QP data. */
  if (setupQPdata(_H, 0, _g, _A, _lb, _ub, _lbA, _ubA) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* 2) Call to main initialisation routine (without any additional information). */
  return solveInitialQP(0, yOpt, 0, 0, nWSR, cputime);
}

returnValue QProblem::init(const real_t* const _H, const real_t* const _R, const real_t* const _g,
                           const real_t* const _A, const real_t* const _lb, const real_t* const _ub,
                           const real_t* const _lbA, const real_t* const _ubA, int& nWSR, const real_t* const yOpt,
                           real_t* const cputime) {
  /* 1) Setup QP data. */
  if (setupQPdata(_H, _R, _g, _A, _lb, _ub, _lbA, _ubA) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblem::hotstart(const real_t* const g_new, const real_t* const lb_new, const real_t* const ub_new,
                               const real_t* const lbA_new, const real_t* const ubA_new, int& nWSR,
                               real_t* const cputime) {
  int l;
  ...
  if (cputime != 0) starttime = getCPUtime();
  ...
  for (l = 0; l < nWSR; ++l) {
    ...
    if (printlevel == PL_HIGH) {
      ...
      getGlobalMessageHandler()->throwInfo(RET_ITERATION_STARTED, messageString, __FUNCTION__, __FILE__, __LINE__,
                                           VS_VISIBLE);
#endif
    }

    /* 1) Setup index arrays. */
    if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_HOTSTART_FAILED);

    if (bounds.getFixed()->getNumberArray(FX_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_HOTSTART_FAILED);

    if (constraints.getActive()->getNumberArray(AC_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_HOTSTART_FAILED);

    if (constraints.getInactive()->getNumberArray(IAC_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_HOTSTART_FAILED);

    /* 2) Detemination of shift direction of the gradient and the (constraints') bounds. */
    returnvalue =
      hotstart_determineDataShift(FX_idx, AC_idx, g_new, lbA_new, ubA_new, lb_new, ub_new, delta_g, delta_lbA,
                                  delta_ubA, delta_lb, delta_ub, Delta_bC_isZero, Delta_bB_isZero);
    ...
    returnvalue =
      hotstart_determineStepDirection(FR_idx, FX_idx, AC_idx, delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub,
                                      Delta_bC_isZero, Delta_bB_isZero, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue =
      hotstart_determineStepLength(FR_idx, FX_idx, AC_idx, IAC_idx, delta_lbA, delta_ubA, delta_lb, delta_ub, delta_xFX,
                                   delta_xFR, delta_yAC, delta_yFX, delta_Ax, BC_idx, BC_status, BC_isBound);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue =
      hotstart_performStep(FR_idx, FX_idx, AC_idx, IAC_idx, delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub,
                           delta_xFX, delta_xFR, delta_yAC, delta_yFX, delta_Ax, BC_idx, BC_status, BC_isBound);

    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;

      /* stop runtime measurement */
      if (cputime != 0) *cputime = getCPUtime() - starttime;

      /* optimal solution found? */
      if (returnvalue == RET_OPTIMAL_SOLUTION_FOUND) {
        status = QPS_SOLVED;

        if (printlevel == PL_HIGH) THROWINFO(RET_OPTIMAL_SOLUTION_FOUND);
  ...
  if (printlevel == PL_HIGH) {
    ...
    return getGlobalMessageHandler()->throwWarning(RET_MAX_NWSR_REACHED, messageString, __FUNCTION__, __FILE__,
                                                   __LINE__, VS_VISIBLE);
#endif
  }
...
returnValue QProblem::getDualSolution(real_t* const yOpt) const {
  ...
  if ((getStatus() == QPS_AUXILIARYQPSOLVED) || (getStatus() == QPS_HOMOTOPYQPSOLVED) || (getStatus() == QPS_SOLVED)) {
    for (i = 0; i < getNV() + getNC(); ++i) yOpt[i] = y[i];
...
returnValue QProblem::setupSubjectToType() {
  ...
  if (QProblemB::setupSubjectToType() != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUPSUBJECTTOTYPE_FAILED);
...
returnValue QProblem::setupCholeskyDecompositionProjected() {
  ...
  for (i = 0; i < nV; ++i)
    for (j = 0; j < nV; ++j) R[i * NVMAX + j] = 0.0;
  ...
  if (hessianType == HST_IDENTITY) {
    ...
    for (i = 0; i < nV; ++i) R[i * NVMAX + i] = 1.0;
  } else {
    if (nZ > 0) {
      int FR_idx[NVMAX];
      if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_INDEXLIST_CORRUPTED);
      ...
      for (j = 0; j < nZ; ++j) {
        ...
        for (i = 0; i < NVMAX; ++i) ZHZ[i] = Q[i * NVMAX + j];
        ...
        for (k = (j - 1); k >= 0; --k) sum -= R[k * NVMAX + j] * R[k * NVMAX + j];
        ...
        for (i = (j + 1); i < nZ; ++i) {
          ...
          for (k = (j - 1); k >= 0; --k) sum -= R[k * NVMAX + j] * R[k * NVMAX + i];
...
returnValue QProblem::solveInitialQP(const real_t* const xOpt, const real_t* const yOpt,
  ...
  if (checkForIdentityHessian() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);

  /* 2) Setup type of bounds and constraints (i.e. unbounded, implicitly fixed etc.). */
  if (setupSubjectToType() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
  ...
  if (bounds.setupAllFree() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);

  if (constraints.setupAllInactive() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);

  /* 2) Setup optimal primal/dual solution for auxiliary QP. */
  if (setupAuxiliaryQPsolution(xOpt, yOpt) != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
  ...
  if (obtainAuxiliaryWorkingSet(xOpt, yOpt, guessedBounds, guessedConstraints, &auxiliaryBounds,
  ...
  if (setupTQfactorisation() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED_TQ);
  ...
  if ((getNAC() + getNFX()) == 0) {
    ...
    if (hasCholesky == BT_FALSE)
      if (setupCholeskyDecomposition() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED_CHOLESKY);
    ...
    if (setupCholeskyDecompositionProjected() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED_CHOLESKY);
  ...
  if (setupAuxiliaryQPgradient() != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
  ...
  if (isInfeasible() == BT_TRUE) return THROWERROR(RET_INIT_FAILED_INFEASIBILITY);

  if (isUnbounded() == BT_TRUE) return THROWERROR(RET_INIT_FAILED_UNBOUNDEDNESS);
  ...
  if (printlevel == PL_HIGH) THROWINFO(RET_INIT_SUCCESSFUL);
...
returnValue QProblem::obtainAuxiliaryWorkingSet(const real_t* const xOpt, const real_t* const yOpt,
  ...
  if ((auxiliaryBounds == 0) || (auxiliaryBounds == guessedBounds)) return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  if (setupAfresh == BT_FALSE) {
    ...
    for (i = 0; i < nC; ++i) {
      if ((constraints.getStatus(i) == ST_LOWER) && (auxiliaryConstraints->getStatus(i) != ST_LOWER))
        if (removeConstraint(i, updateCholesky) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
    ...
    for (i = 0; i < nV; ++i) {
      if ((bounds.getStatus(i) == ST_LOWER) && (auxiliaryBounds->getStatus(i) != ST_LOWER))
        if (removeBound(i, updateCholesky) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
...
returnValue QProblem::setupAuxiliaryQPsolution(const real_t* const xOpt, const real_t* const yOpt) {
  ...
  if (xOpt != 0) {
    if (xOpt != x)
      for (i = 0; i < nV; ++i) x[i] = xOpt[i];

    for (j = 0; j < nC; ++j) {
      Ax[j] = 0.0;

      for (i = 0; i < nV; ++i) Ax[j] += A[j * NVMAX + i] * x[i];
    }
  } else {
    for (i = 0; i < nV; ++i) x[i] = 0.0;

    for (j = 0; j < nC; ++j) Ax[j] = 0.0;
  }

  if (yOpt != 0) {
    if (yOpt != y)
      for (i = 0; i < nV + nC; ++i) y[i] = yOpt[i];
  } else {
    for (i = 0; i < nV + nC; ++i) y[i] = 0.0;
  }
...
returnValue QProblem::setupAuxiliaryQPgradient() {
  ...
  for (i = 0; i < nV; ++i) {
    ...
    /* A'*yC */
    for (j = 0; j < nC; ++j) g[i] += A[j * NVMAX + i] * y[nV + j];

    /* -H*x */
    for (j = 0; j < nV; ++j) g[i] -= H[i * NVMAX + j] * x[j];
  }
...
returnValue QProblem::setupAuxiliaryQPbounds(const Bounds* const auxiliaryBounds,
                                             const Constraints* const auxiliaryConstraints, BooleanType useRelaxation) {
  int i;
  int nV = getNV();
  int nC = getNC();
    ...
    switch (bounds.getStatus(i)) {
      ...
      case ST_LOWER:
        ...
        if (bounds.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) ub[i] = x[i] + BOUNDRELAXATION;
        }
        break;
      ...
      case ST_UPPER:
        ...
        if (bounds.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) lb[i] = x[i] - BOUNDRELAXATION;
        }
        break;
    ...
    switch (constraints.getStatus(i)) {
      ...
      case ST_LOWER:
        ...
        if (constraints.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) ubA[i] = Ax[i] + BOUNDRELAXATION;
        }
        break;
      ...
      case ST_UPPER:
        ...
        if (constraints.getType(i) == ST_EQUALITY) {
          ...
          if (useRelaxation == BT_TRUE) lbA[i] = Ax[i] - BOUNDRELAXATION;
        }
        break;
 ...
 */
returnValue QProblem::addConstraint(int number, SubjectToStatus C_status, BooleanType updateCholesky) {
  int i, j, ii;

  /* consistency checks */
  if (constraints.getStatus(number) != ST_INACTIVE) return THROWERROR(RET_CONSTRAINT_ALREADY_ACTIVE);

  if ((constraints.getNC() - getNAC()) == constraints.getNUC()) return THROWERROR(RET_ALL_CONSTRAINTS_ACTIVE);
  ...
  int FR_idx[NVMAX];
  if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDCONSTRAINT_FAILED);
  ...
  for (i = 0; i < nFR; ++i) {
    ii = FR_idx[i];
    for (j = 0; j < nZ; ++j) wZ[j] += aFR[i] * Q[ii * NVMAX + j];
  }

  /* 2) Calculate wY and store it directly into T. */
  if (nAC > 0) {
    for (j = 0; j < nAC; ++j) T[nAC * NVMAX + tcol + j] = 0.0;
    for (i = 0; i < nFR; ++i) {
      ii = FR_idx[i];
      for (j = 0; j < nAC; ++j) T[nAC * NVMAX + tcol + j] += aFR[i] * Q[ii * NVMAX + nZ + j];
    }
  }
  ...
  if (nZ > 0) {
    ...
    if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
      ...
      for (i = 0; i < nZ; ++i) R[i * NVMAX + nZ - 1] = 0.0;
    }
  }
...
returnValue QProblem::addConstraint_checkLI(int number) {
  ...
  for (i = 0; i < nZ; ++i) {
    ...
    if (getAbs(sum) > 10.0 * EPS) return RET_LINEARLY_INDEPENDENT;
  }
...
returnValue QProblem::addConstraint_ensureLI(int number, SubjectToStatus C_status) {
  ...
  if (returnvalueCheckLI == RET_INDEXLIST_CORRUPTED) return THROWERROR(RET_ENSURELI_FAILED);
  ...
  int FR_idx[NVMAX];
  if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_ENSURELI_FAILED);

  int FX_idx[NVMAX];
  if (bounds.getFixed()->getNumberArray(FX_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_ENSURELI_FAILED);
  ...
  if (nAC > 0) {
    ...
    if (backsolveT(xiC_TMP, BT_TRUE, xiC) != SUCCESSFUL_RETURN) return THROWERROR(RET_ENSURELI_FAILED_TQ);
  }

  /* 3) Calculate xiB. */
  int AC_idx[NCMAX_ALLOC];
  if (constraints.getActive()->getNumberArray(AC_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_ENSURELI_FAILED);
...
/* setup output preferences */
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET, messageString, __FUNCTION__, __FILE__, __LINE__,
                                           visibilityStatus);
#endif

      if (removeBound(y_min_number, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVE_FROM_ACTIVESET_FAILED);
...
returnValue QProblem::addBound(int number, SubjectToStatus B_status, BooleanType updateCholesky) {
  ...
  /* consistency checks */
  if (bounds.getStatus(number) != ST_INACTIVE) return THROWERROR(RET_BOUND_ALREADY_ACTIVE);
  ...
  if (lastfreenumber != number)
    if (bounds.swapFree(number, lastfreenumber) != SUCCESSFUL_RETURN) THROWERROR(RET_ADDBOUND_FAILED);

  int FR_idx[NVMAX];
  if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDBOUND_FAILED);

  real_t w[NVMAX];

  /* II) ADD NEW ACTIVE BOUND TO TOP OF MATRIX T: */
  /* 1) add row [wZ wY] = [Z Y](number) at the top of T: assign w */
  for (i = 0; i < nFR; ++i) w[i] = Q[FR_idx[nFR - 1] * NVMAX + i];
  ...
  /* IV) UPDATE INDICES */
  if (bounds.moveFreeToFixed(number, B_status) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADDBOUND_FAILED);
...
returnValue QProblem::addBound_checkLI(int number) {
  ...
  /* Check if constraint <number> is linearly independent from the
     the active ones (<=> is element of null space of Afr). */
  for (i = 0; i < nZ; ++i) {
    if (getAbs(Q[number * NVMAX + i]) > EPS) return RET_LINEARLY_INDEPENDENT;
  }
...
returnValue QProblem::addBound_ensureLI(int number, SubjectToStatus B_status) {
  ...
  if (nAC > 0) {
    if (B_status == ST_LOWER) {
      for (i = 0; i < nAC; ++i) xiC_TMP[i] = Q[number * NVMAX + nZ + i];
    } else {
      for (i = 0; i < nAC; ++i) xiC_TMP[i] = -Q[number * NVMAX + nZ + i];
    }
...
returnValue QProblem::removeConstraint(int number, BooleanType updateCholesky) {
  ...
  /* consistency checks */
  if (constraints.getStatus(number) == ST_INACTIVE) return THROWERROR(RET_CONSTRAINT_NOT_ACTIVE);

  if ((number_idx < 0) || (number_idx >= nAC)) return THROWERROR(RET_CONSTRAINT_NOT_ACTIVE);

  int FR_idx[NVMAX];
  if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
  ...
  if (number_idx < nAC - 1) {
    for (i = (number_idx + 1); i < nAC; ++i)
      for (j = (nAC - i - 1); j < nAC; ++j) T[(i - 1) * NVMAX + tcol + j] = T[i * NVMAX + tcol + j];
    /* gimmick: write zeros into the last row of T */
    for (j = 0; j < nAC; ++j) T[(nAC - 1) * NVMAX + tcol + j] = 0.0;
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(T[(nAC - 2 - j) * NVMAX + tcol + 1 + j], T[(nAC - 2 - j) * NVMAX + tcol + j],
                    T[(nAC - 2 - j) * NVMAX + tcol + 1 + j], T[(nAC - 2 - j) * NVMAX + tcol + j], c, s);

      for (i = (nAC - j - 1); i < (nAC - 1); ++i)
        applyGivens(c, s, T[i * NVMAX + tcol + 1 + j], T[i * NVMAX + tcol + j], T[i * NVMAX + tcol + 1 + j],
                    T[i * NVMAX + tcol + j]);

      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, Q[ii * NVMAX + nZ + 1 + j], Q[ii * NVMAX + nZ + j], Q[ii * NVMAX + nZ + 1 + j],
                    Q[ii * NVMAX + nZ + j]);
      }
    }
  } else {
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
    ...
    for (j = 0; j < nFR; ++j) {
      jj = FR_idx[j];
      for (i = 0; i < nFR; ++i) Hz[i] += H[jj * NVMAX + FR_idx[i]] * Q[jj * NVMAX + nZ];
    }
    ...
    if (nZ > 0) {
      ...
      for (j = 0; j < nFR; ++j) {
        ...
        for (i = 0; i < nZ; ++i) ZHz[i] += Q[jj * NVMAX + i] * Hz[j];
      }

      /* 3) Calculate r = R^-T * ZHz. */
      if (backsolveR(ZHz, BT_TRUE, r) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
    ...
    for (j = 0; j < nFR; ++j) rho2 += Q[FR_idx[j] * NVMAX + nZ] * Hz[j];
  ...
  /* IV) UPDATE INDICES */
  if (constraints.moveActiveToInactive(number) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
...
returnValue QProblem::removeBound(int number, BooleanType updateCholesky) {
  ...
  /* consistency checks */
  if (bounds.getStatus(number) == ST_INACTIVE) return THROWERROR(RET_BOUND_NOT_ACTIVE);
  ...
  /* I) UPDATE INDICES */
  if (bounds.moveFixedToFree(number) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);

  int FR_idx[NVMAX];
  if (bounds.getFree()->getNumberArray(FR_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
  ...
  if (nAC > 0) {
    ...
    int AC_idx[NCMAX_ALLOC];
    if (constraints.getActive()->getNumberArray(AC_idx) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      computeGivens(tmp[nAC - 1 - j], T[(nAC - 1 - j) * NVMAX + tcol + j], T[(nAC - 1 - j) * NVMAX + tcol + j],
                    tmp[nAC - 1 - j], c, s);
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_IDENTITY)) {
    ...
    if (nFR > 0) {
      ...
      if (nZ > 0) {
        ...
        if (backsolveR(rhs, BT_TRUE, BT_TRUE, r) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVEBOUND_FAILED);
 ...
 *	b a c k s o l v e R  (CODE DUPLICATE OF QProblemB CLASS!!!)
 */
returnValue QProblem::backsolveR(const real_t* const b, BooleanType transposed, BooleanType removingBound,
                                 real_t* const a) {
  int i, j;
  int nR = getNZ();
  ...
  if (transposed == BT_FALSE) {
    ...
    for (i = (nR - 1); i >= 0; --i) {
      sum = b[i];
      for (j = (i + 1); j < nR; ++j) sum -= R[i * NVMAX + j] * a[j];
    ...
    for (i = 0; i < nR; ++i) {
      ...
      for (j = 0; j < i; ++j) sum -= R[j * NVMAX + i] * a[j];
...
returnValue QProblem::backsolveT(const real_t* const b, BooleanType transposed, real_t* const a) {
  ...
  if (transposed == BT_FALSE) {
    ...
    for (i = 0; i < nT; ++i) {
      sum = b[i];
      for (j = 0; j < i; ++j) sum -= T[i * NVMAX + sizeT - 1 - j] * a[nT - 1 - j];
    ...
    for (i = 0; i < nT; ++i) {
      sum = b[i];
      for (j = 0; j < i; ++j) sum -= T[(nT - 1 - j) * NVMAX + tcol + i] * a[nT - 1 - j];
...
returnValue QProblem::hotstart_determineDataShift(const int* const FX_idx, const int* const AC_idx,
                                                  ...
                                                  const real_t* const ub_new, real_t* const delta_g,
                                                  real_t* const delta_lbA, real_t* const delta_ubA,
 ...
 */
returnValue QProblem::hotstart_determineStepDirection(const int* const FR_idx, const int* const FX_idx,
                                                      const int* const AC_idx, const real_t* const delta_g,
  ...
  if (nFR > 0) {
    ...
    if (nAC > 0) {
      if ((Delta_bC_isZero == BT_TRUE) && (Delta_bB_isZero == BT_TRUE)) {
        ...
        for (i = 0; i < nFR; ++i) {
          ii = FR_idx[i];
          for (j = 0; j < nAC; ++j) YFR_delta_xFRy[i] += Q[ii * NVMAX + nZ + j] * delta_xFRy[j];
    ...
    if (hessianType == HST_IDENTITY) {
      for (j = 0; j < nFR; ++j) {
        jj = FR_idx[j];
        for (i = 0; i < nZ; ++i) delta_xFRz[i] -= Q[jj * NVMAX + i] * delta_g[jj];
      }

      if (nZ > 0) {
        for (i = 0; i < nFR; ++i) {
          ii = FR_idx[i];
          for (j = 0; j < nZ; ++j) ZFR_delta_xFRz[i] += Q[ii * NVMAX + j] * delta_xFRz[j];
      ...
      if (nZ > 0) {
        ...
        for (j = 0; j < nFR; ++j) {
          jj = FR_idx[j];
          for (i = 0; i < nZ; ++i) delta_xFRz[i] -= Q[jj * NVMAX + i] * delta_xFRz_RHS[j];
        }
 ...
 */
returnValue QProblem::hotstart_determineStepLength(const int* const FR_idx, const int* const FX_idx,
                                                   const int* const AC_idx, const int* const IAC_idx,
                                                   const real_t* const delta_lbA, const real_t* const delta_ubA,
                                                   const real_t* const delta_lb, const real_t* const delta_ub,
                                                   const real_t* const delta_xFX, const real_t* const delta_xFR,
                                                   const real_t* const delta_yAC, const real_t* const delta_yFX,
                                                   real_t* const delta_Ax, int& BC_idx, SubjectToStatus& BC_status,
                                                   BooleanType& BC_isBound) {
  int i, j, ii, jj;
  int nV = getNV();
  int nC = getNC();
  ...
  for (i = 0; i < nIAC; ++i) {
    ...
    if (constraints.getType(ii) != ST_UNBOUNDED) {
      delta_Ax[ii] = 0.0;
      for (j = 0; j < nV; ++j) delta_Ax[ii] += A[ii * NVMAX + j] * delta_x[j];  // POSSIBLE SPEEDUP!
  ...
  if (tau_new > EPS) cyclingManager.clearCyclingData();
  ...
  if (printlevel == PL_HIGH) {
    ...
    if (BC_status == ST_UNDEFINED)
      ...
      sprintf(messageString, "Stepsize is %.6e! (BC_idx = %d, BC_isBound = %d, BC_status = %d)", tau, BC_idx,
              BC_isBound, BC_status);

    getGlobalMessageHandler()->throwInfo(RET_STEPSIZE_NONPOSITIVE, messageString, __FUNCTION__, __FILE__, __LINE__,
                                         VS_VISIBLE);
  }
#endif
 ...
 */
returnValue QProblem::hotstart_performStep(const int* const FR_idx, const int* const FX_idx, const int* const AC_idx,
  ...
  if (tau > ZERO) {
    ...
    for (i = 0; i < nAC; ++i) {
      ...
      Ax[ii] = 0.0;
      for (j = 0; j < nV; ++j) Ax[ii] += A[ii * NVMAX + j] * x[j];
    }
  ...
  switch (BC_status) {
    ...
    case ST_INACTIVE:
      if (BC_isBound == BT_TRUE) {
        ...
        if (removeBound(BC_idx, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVE_FROM_ACTIVESET_FAILED);
        ...
        if (removeConstraint(BC_idx, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_REMOVE_FROM_ACTIVESET_FAILED);
    ...
    default:
      if (BC_isBound == BT_TRUE) {
        ...
        getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString, __FUNCTION__, __FILE__, __LINE__,
                                             visibilityStatus);
#endif

        if (addBound(BC_idx, BC_status, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_ADD_TO_ACTIVESET_FAILED);
      } else {
#ifdef PC_DEBUG
        if (BC_status == ST_LOWER)
...
BooleanType QProblem::areBoundsConsistent(const real_t* const delta_lb, const real_t* const delta_ub,
                                          const real_t* const delta_lbA, const real_t* const delta_ubA) const {
  int i;

  /* 1) Check bounds' consistency. */
  if (QProblemB::areBoundsConsistent(delta_lb, delta_ub) == BT_FALSE) return BT_FALSE;
  ...
  for (i = 0; i < getNC(); ++i)
    if ((lbA[i] > ubA[i] - BOUNDTOL) && (delta_lbA[i] > delta_ubA[i] + EPS)) return BT_FALSE;
 ...
 */
returnValue QProblem::setupQPdata(const real_t* const _H, const real_t* const _R, const real_t* const _g,
                                  const real_t* const _A, const real_t* const _lb, const real_t* const _ub,
  ...
  if (QProblemB::setupQPdata(_H, _R, _g, _lb, _ub) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* 2) Load constraint matrix. */
  if ((nC > 0) && (_A == 0)) return THROWERROR(RET_INVALID_ARGUMENTS);

  if (nC > 0) {
    for (i = 0; i < nC; ++i)
      for (j = 0; j < nV; ++j) A[i * NVMAX + j] = _A[i * nV + j];

    /* 3) Setup lower constraints' bounds vector. */
    if (_lbA != 0) {
      for (i = 0; i < nC; ++i) lbA[i] = _lbA[i];
    } else {
      /* if no lower constraints' bounds are specified, set them to -infinity */
      for (i = 0; i < nC; ++i) lbA[i] = -INFTY;
    }

    /* 4) Setup upper constraints' bounds vector. */
    if (_ubA != 0) {
      for (i = 0; i < nC; ++i) ubA[i] = _ubA[i];
    } else {
      /* if no upper constraints' bounds are specified, set them to infinity */
      for (i = 0; i < nC; ++i) ubA[i] = INFTY;
    }
  }
 ...
 */
returnValue QProblem::printIteration(int iteration, int BC_idx, SubjectToStatus BC_status, BooleanType BC_isBound) {
  char myPrintfString[160];

  /* consistency check */
  if (iteration < 0) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* nothing to do */
  if (printlevel != PL_MEDIUM) return SUCCESSFUL_RETURN;
...
returnValue QProblem::checkKKTconditions() {
  ...
  for (i = 0; i < nV; ++i) {
    ...
    for (j = 0; j < nV; ++j) tmp += H[i * NVMAX + j] * x[j];
    ...
    if (getAbs(tmp) > maxKKTviolation) maxKKTviolation = getAbs(tmp);
  }
  ...
  for (i = 0; i < nV; ++i) {
    switch (bounds.getStatus(i)) {
      case ST_LOWER:
        ...
        if (getAbs(x[i] - lb[i]) > maxKKTviolation) maxKKTviolation = getAbs(x[i] - lb[i]);
        break;

      case ST_UPPER:
        if (y[i] > maxKKTviolation) maxKKTviolation = y[i];
        if (getAbs(ub[i] - x[i]) > maxKKTviolation) maxKKTviolation = getAbs(ub[i] - x[i]);
        break;

      default: /* inactive */
        if (getAbs(y[i]) > maxKKTviolation) maxKKTviolation = getAbs(y[i]);
        break;
    }
  }
  ...
  for (i = 0; i < nC; ++i) {
    switch (constraints.getStatus(i)) {
      case ST_LOWER:
        ...
        if (getAbs(Ax[i] - lbA[i]) > maxKKTviolation) maxKKTviolation = getAbs(Ax[i] - lbA[i]);
        break;

      case ST_UPPER:
        if (y[nV + i] > maxKKTviolation) maxKKTviolation = y[nV + i];
        if (getAbs(ubA[i] - Ax[i]) > maxKKTviolation) maxKKTviolation = getAbs(ubA[i] - Ax[i]);
        break;

      default: /* inactive */
        if (getAbs(y[nV + i]) > maxKKTviolation) maxKKTviolation = getAbs(y[nV + i]);
        break;
    }
  }
