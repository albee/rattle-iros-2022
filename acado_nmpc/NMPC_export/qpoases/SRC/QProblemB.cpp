...
QProblemB::QProblemB(const QProblemB& rhs) {
  ...
  for (i = 0; i < _nV; ++i)
    for (j = 0; j < _nV; ++j) H[i * NVMAX + j] = rhs.H[i * NVMAX + j];

  hasHessian = rhs.hasHessian;

  for (i = 0; i < _nV; ++i) g[i] = rhs.g[i];

  for (i = 0; i < _nV; ++i) lb[i] = rhs.lb[i];

  for (i = 0; i < _nV; ++i) ub[i] = rhs.ub[i];

  bounds = rhs.bounds;

  for (i = 0; i < _nV; ++i)
    for (j = 0; j < _nV; ++j) R[i * NVMAX + j] = rhs.R[i * NVMAX + j];
  hasCholesky = rhs.hasCholesky;

  for (i = 0; i < _nV; ++i) x[i] = rhs.x[i];

  for (i = 0; i < _nV; ++i) y[i] = rhs.y[i];
 ...
 *	i n i t
 */
returnValue QProblemB::init(const real_t* const _H, const real_t* const _g, const real_t* const _lb,
                            const real_t* const _ub, int& nWSR, const real_t* const yOpt, real_t* const cputime) {
  /* 1) Setup QP data. */
  if (setupQPdata(_H, 0, _g, _lb, _ub) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblemB::init(const real_t* const _H, const real_t* const _R, const real_t* const _g,
                            const real_t* const _lb, const real_t* const _ub, int& nWSR, const real_t* const yOpt,
                            real_t* const cputime) {
  /* 1) Setup QP data. */
  if (setupQPdata(_H, _R, _g, _lb, _ub) != SUCCESSFUL_RETURN) return THROWERROR(RET_INVALID_ARGUMENTS);
...
returnValue QProblemB::hotstart(const real_t* const g_new, const real_t* const lb_new, const real_t* const ub_new,
  ...
  for (l = 0; l < nWSR; ++l) {
    ...
    returnvalue =
      hotstart_determineDataShift(FX_idx, g_new, lb_new, ub_new, delta_g, delta_lb, delta_ub, Delta_bB_isZero);
    ...
    returnvalue = hotstart_determineStepDirection(FR_idx, FX_idx, delta_g, delta_lb, delta_ub, Delta_bB_isZero,
    ...
    returnvalue =
      hotstart_determineStepLength(FR_idx, FX_idx, delta_lb, delta_ub, delta_xFR, delta_yFX, BC_idx, BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      nWSR = l;
    ...
    returnvalue = hotstart_performStep(FR_idx, FX_idx, delta_g, delta_lb, delta_ub, delta_xFX, delta_xFR, delta_yFX,
                                       BC_idx, BC_status);
...
real_t QProblemB::getObjVal(const real_t* const _x) const {
  ...
  for (i = 0; i < nV; ++i) {
    ...
    for (j = 0; j < nV; ++j) obj_tmp += 0.5 * _x[i] * H[i * NVMAX + j] * _x[j];
  }
...
#ifndef __MATLAB__
  if ((printlevel >= PL_MEDIUM) && (printlevel != _printlevel)) THROWINFO(RET_PRINTLEVEL_CHANGED);
#endif
...
returnValue QProblemB::checkForIdentityHessian() {
  ...
  for (i = 0; i < nV; ++i)
    if (getAbs(H[i * NVMAX + i] - 1.0) > EPS) return SUCCESSFUL_RETURN;

  for (i = 0; i < nV; ++i) {
    for (j = 0; j < i; ++j)
      if ((getAbs(H[i * NVMAX + j]) > EPS) || (getAbs(H[j * NVMAX + i]) > EPS)) return SUCCESSFUL_RETURN;
  }
...
returnValue QProblemB::setupCholeskyDecomposition() {
  ...
  if (hasHessian == BT_FALSE) return SUCCESSFUL_RETURN;
  ...
  if (hessianType == HST_IDENTITY) {
    ...
    for (i = 0; i < nFR; ++i) R[i * NVMAX + i] = 1.0;
  } else {
    if (nFR > 0) {
      int FR_idx[NVMAX];
      ...
      for (i = 0; i < nFR; ++i) {
        ...
        for (k = (i - 1); k >= 0; --k) sum -= R[k * NVMAX + i] * R[k * NVMAX + i];
        ...
        for (j = (i + 1); j < nFR; ++j) {
          ...
          for (k = (i - 1); k >= 0; --k) sum -= R[k * NVMAX + i] * R[k * NVMAX + j];
...
returnValue QProblemB::solveInitialQP(const real_t* const xOpt, const real_t* const yOpt,
                                      const Bounds* const guessedBounds, int& nWSR, real_t* const cputime) {
  int i, nFR;
  int nV = getNV();
  ...
  if (setupAuxiliaryWorkingSet(&auxiliaryBounds, BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
  ...
  for (i = 0; i < nV; ++i) g_original[i] = g[i];
  for (i = 0; i < nV; ++i) lb_original[i] = lb[i];
  for (i = 0; i < nV; ++i) ub_original[i] = ub[i];
  ...
  if (setupAuxiliaryQPbounds(BT_TRUE) != SUCCESSFUL_RETURN) return THROWERROR(RET_INIT_FAILED);
 ...
 */
returnValue QProblemB::setupAuxiliaryWorkingSet(const Bounds* const auxiliaryBounds, BooleanType setupAfresh) {
  int i;
  int nV = getNV();
...
returnValue QProblemB::setupAuxiliaryQPsolution(const real_t* const xOpt, const real_t* const yOpt) {
  ...
  if (yOpt != 0) {
    if (yOpt != y)
      for (i = 0; i < nV; ++i) y[i] = yOpt[i];
  } else {
    for (i = 0; i < nV; ++i) y[i] = 0.0;
  }
...
returnValue QProblemB::addBound(int number, SubjectToStatus B_status, BooleanType updateCholesky) {
  ...
  for (i = 0; i < nFR - 1; ++i)
    for (j = number_idx + 1; j < nFR; ++j) R[i * NVMAX + j - 1] = R[i * NVMAX + j];
  /* ... last column of R. */
  for (i = 0; i < nFR; ++i) R[i * NVMAX + nFR - 1] = 0.0;
...
returnValue QProblemB::removeBound(int number, BooleanType updateCholesky) {
  ...
  /* Perform cholesky updates only if QProblemB has been initialised! */
  if ((getStatus() == QPS_PREPARINGAUXILIARYQP) || (updateCholesky == BT_FALSE)) return SUCCESSFUL_RETURN;
  ...
  for (i = 0; i < nFR; ++i) R[i * NVMAX + nFR] = r[i];
 ...
 *	b a c k s o l v e R  (CODE DUPLICATED IN QProblem CLASS!!!)
 */
returnValue QProblemB::backsolveR(const real_t* const b, BooleanType transposed, BooleanType removingBound,
                                  real_t* const a) {
  int i, j;
  int nR = getNZ();
 ...
 */
returnValue QProblemB::hotstart_determineDataShift(const int* const FX_idx, const real_t* const g_new,
                                                   const real_t* const lb_new, const real_t* const ub_new,
                                                   real_t* const delta_g, real_t* const delta_lb,
 ...
 */
BooleanType QProblemB::areBoundsConsistent(const real_t* const delta_lb, const real_t* const delta_ub) const {
  int i;

  /* Check if delta_lb[i] is greater than delta_ub[i]
   * for a component i whose bounds are already (numerically) equal. */
  for (i = 0; i < getNV(); ++i)
    if ((lb[i] > ub[i] - BOUNDTOL) && (delta_lb[i] > delta_ub[i] + EPS)) return BT_FALSE;
...
returnValue QProblemB::setupQPdata(const real_t* const _H, const real_t* const _R, const real_t* const _g,
  ...
  if (_H != 0) {
    for (i = 0; i < nV; ++i)
      for (j = 0; j < nV; ++j) H[i * NVMAX + j] = _H[i * nV + j];
    hasHessian = BT_TRUE;
  } else
    hasHessian = BT_FALSE;
  ...
  if (_R != 0) {
    for (i = 0; i < nV; ++i)
      for (j = 0; j < nV; ++j) R[i * NVMAX + j] = _R[i * nV + j];
    hasCholesky = BT_TRUE;
    ...
    if (hasHessian == BT_FALSE)
      for (i = 0; i < nV; ++i)
        for (j = 0; j < nV; ++j) H[i * NVMAX + j] = _R[i * nV + j];
  } else
    hasCholesky = BT_FALSE;

  if (hasHessian == BT_FALSE && hasCholesky == BT_FALSE) return THROWERROR(RET_INVALID_ARGUMENTS);

  /* 2) Setup gradient vector. */
  if (_g == 0) return THROWERROR(RET_INVALID_ARGUMENTS);

  for (i = 0; i < nV; ++i) g[i] = _g[i];

  /* 3) Setup lower bounds vector. */
  if (_lb != 0) {
    for (i = 0; i < nV; ++i) lb[i] = _lb[i];
  } else {
    /* if no lower bounds are specified, set them to -infinity */
    for (i = 0; i < nV; ++i) lb[i] = -INFTY;
  }

  /* 4) Setup upper bounds vector. */
  if (_ub != 0) {
    for (i = 0; i < nV; ++i) ub[i] = _ub[i];
  } else {
    /* if no upper bounds are specified, set them to infinity */
    for (i = 0; i < nV; ++i) ub[i] = INFTY;
  }
...
returnValue QProblemB::hotstart_determineStepLength(const int* const FR_idx, const int* const FX_idx,
                                                    ...
                                                    const real_t* const delta_xFR, const real_t* const delta_yFX,
  ...
  for (i = 0; i < nV; ++i) {
    switch (bounds.getStatus(i)) {
      case ST_LOWER:
        ...
        if (getAbs((x[i] - lb[i]) * y[i]) > maxKKTviolation) maxKKTviolation = getAbs((x[i] - lb[i]) * y[i]);
        break;

      case ST_UPPER:
        if (y[i] > maxKKTviolation) maxKKTviolation = y[i];
        if (getAbs((ub[i] - x[i]) * y[i]) > maxKKTviolation) maxKKTviolation = getAbs((ub[i] - x[i]) * y[i]);
        break;
