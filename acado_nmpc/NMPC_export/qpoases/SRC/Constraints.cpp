...
Constraints::Constraints(const Constraints& rhs)
    : SubjectTo(rhs), nC(rhs.nC), nEC(rhs.nEC), nIC(rhs.nIC), nUC(rhs.nUC) {
  active = rhs.active;
...
returnValue Constraints::setupConstraint(int _number, SubjectToStatus _status) {
  /* consistency check */
  if ((_number < 0) || (_number >= getNC())) return THROWERROR(RET_INDEX_OUT_OF_BOUNDS);
...
returnValue Constraints::setupAllInactive() {
  ...
  for (i = 0; i < nC; ++i) {
    if (getType(i) == ST_UNBOUNDED) {
      if (setupConstraint(i, ST_INACTIVE) != SUCCESSFUL_RETURN) return THROWERROR(RET_SETUP_CONSTRAINT_FAILED);
    }
  }
...
returnValue Constraints::moveActiveToInactive(int _number) {
  ...
  if (this->removeIndex(this->getActive(), _number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Constraints::moveInactiveToActive(int _number, SubjectToStatus _status) {
  ...
  if (this->removeIndex(this->getInactive(), _number) != SUCCESSFUL_RETURN) return THROWERROR(RET_MOVING_BOUND_FAILED);
