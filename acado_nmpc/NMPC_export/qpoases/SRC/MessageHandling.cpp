 ...
 *	M e s s a g e H a n d l i n g
 */
MessageHandling::MessageHandling(VisibilityStatus _errorVisibility, VisibilityStatus _warningVisibility,
 ...
 *	M e s s a g e H a n d l i n g
 */
MessageHandling::MessageHandling(myFILE* _outputFile, VisibilityStatus _errorVisibility,
                                 VisibilityStatus _warningVisibility, VisibilityStatus _infoVisibility)
    : errorVisibility(_errorVisibility),
      warningVisibility(_warningVisibility),
      infoVisibility(_infoVisibility),
      outputFile(_outputFile),
...
MessageHandling::~MessageHandling() {
#ifdef PC_DEBUG
  if (outputFile != 0) fclose(outputFile);
#endif
}
 ...
 *	t h r o w E r r o r
 */
returnValue MessageHandling::throwError(returnValue Enumber, const char* additionaltext, const char* functionname,
 ...
 */
returnValue MessageHandling::throwWarning(returnValue Wnumber, const char* additionaltext, const char* functionname,
                                          const char* filename, const unsigned long linenumber,
                                          VisibilityStatus localVisibilityStatus) {
  /* consistency check */
 ...
 *	t h r o w I n f o
 */
returnValue MessageHandling::throwInfo(returnValue Inumber, const char* additionaltext, const char* functionname,
                                       const char* filename, const unsigned long linenumber,
                                       VisibilityStatus localVisibilityStatus) {
  /* consistency check */
 ...
 */
returnValue MessageHandling::throwMessage(returnValue RETnumber, const char* additionaltext, const char* functionname,
                                          const char* filename, const unsigned long linenumber,
  ...
  if (numberOfWhitespaces > 40) numberOfWhitespaces = 40;

  for (i = 0; i < numberOfWhitespaces; ++i) whitespaces[i] = ' ';
  ...
  if ((returnValueList[keypos].globalVisibilityStatus == VS_VISIBLE) && (localVisibilityStatus == VS_VISIBLE)) {
    ...
    if (additionaltext == 0) {
      sprintf(myPrintfString, "%s (%s, %s:%d): \t%s\n", RETstring, functionname, filename, (int)linenumber,
 ...
 */
MessageHandling* getGlobalMessageHandler() { return &globalMessageHandler; }

const char* MessageHandling::getErrorString(int error) { return returnValueList[error].data; }
