...
std::string call_dlr(std::vector<int> params, std::vector<double> chaser_pos, double target_mass,
  std::vector<double> target_inertia, std::vector<double> target_pos,
  std::vector<double> target_quat, std::vector<double> target_ang_vel, std::string TRAJ_PATH, bool SIM) {
  /*
  Calls DLR's motion planning executable, with the following format:
  ...
  */
  ...
  // hardware testing (where the exe is)
    ...
    CMD = "/opt/roam/bin/";  // hard-coded for ground test!
  }
  /**********************************/

  std::string DLR_EXE_NAME{"mpMIT"};
  CMD += DLR_EXE_NAME;
...
template<typename M>
M get_dlr_output_u(const std::string& path) {
...
template<typename M>
M get_dlr_output_x(const std::string& path){
  /*
