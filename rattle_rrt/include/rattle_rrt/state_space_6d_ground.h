...
class StateSpace6DGround: public StateSpace<STATE_6D> {
...
public:
  ...
  static const int num_T = 5;  // num_T : number of possible torque inputs (motion primitives) to sample from

  RigidBodyDynamics model_;  // the dynamics model to evaluate

  typedef Eigen::Array<STATE_6D, num_F, num_T> X_NEXT_ARRAY;
  typedef Eigen::Matrix<double, 3, num_F> F_ARRAY;
  typedef Eigen::Matrix<double, 3, num_T> T_ARRAY;

  StateSpace6DGround(Eigen::Matrix<double, 6, 1> min_bounds, Eigen::Matrix<double, 6, 1> max_bounds, double mass, double ixx, double iyy, double izz);
  ...
  bool is_segment_valid(const STATE_6D& start, const STATE_6D& goal, const std::vector<Obstacle>& obstacles = std::vector<Obstacle>()) override;

  int get_d() const override;

  // derived class funcs
  void get_min_result(STATE_6D& x_new, Vec3& prev_F, Vec3& prev_T, int& extend_success,
