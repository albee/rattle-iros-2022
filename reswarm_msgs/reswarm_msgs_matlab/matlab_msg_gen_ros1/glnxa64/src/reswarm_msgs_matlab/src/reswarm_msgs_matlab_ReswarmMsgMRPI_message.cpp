...
// Common copy functions for reswarm_msgs_matlab/ReswarmMsgMRPI
...
#endif //_MSC_VER
#include "ros/ros.h"
#include "reswarm_msgs_matlab/ReswarmMsgMRPI.h"
#include "visibility_control.h"
#include "class_loader/multi_library_class_loader.hpp"
#include "MATLABROSMsgInterface.hpp"
#include "ROSPubSubTemplates.hpp"
class RESWARM_MSGS_MATLAB_EXPORT reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common : public MATLABROSMsgInterface<reswarm_msgs_matlab::ReswarmMsgMRPI> {
  public:
    virtual ~reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common(){}
    virtual void copy_from_struct(reswarm_msgs_matlab::ReswarmMsgMRPI* msg, const matlab::data::Struct& arr, MultiLibLoader loader); 
    //----------------------------------------------------------------------------
    virtual MDArray_T get_arr(MDFactory_T& factory, const reswarm_msgs_matlab::ReswarmMsgMRPI* msg, MultiLibLoader loader, size_t size = 1);
};
  void reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common::copy_from_struct(reswarm_msgs_matlab::ReswarmMsgMRPI* msg, const matlab::data::Struct& arr,
  ...
  //----------------------------------------------------------------------------
  MDArray_T reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common::get_arr(MDFactory_T& factory, const reswarm_msgs_matlab::ReswarmMsgMRPI* msg,
       MultiLibLoader loader, size_t size) {
    auto outArray = factory.createStructArray({size,1},{"MessageType","K","Au","Bu","AZ","BZ","UsingFallbackMrpi"});
    for(size_t ctr = 0; ctr < size; ctr++){
    outArray[ctr]["MessageType"] = factory.createCharArray("reswarm_msgs_matlab/ReswarmMsgMRPI");
    // K
...
class RESWARM_MSGS_MATLAB_EXPORT reswarm_msgs_matlab_ReswarmMsgMRPI_message : public ROSMsgElementInterfaceFactory {
  public:
    virtual ~reswarm_msgs_matlab_ReswarmMsgMRPI_message(){}
    virtual std::shared_ptr<MATLABPublisherInterface> generatePublisherInterface(ElementType type);
    virtual std::shared_ptr<MATLABSubscriberInterface> generateSubscriberInterface(ElementType type);
};  
  std::shared_ptr<MATLABPublisherInterface> 
          reswarm_msgs_matlab_ReswarmMsgMRPI_message::generatePublisherInterface(ElementType type){
    if(type != eMessage){
        throw std::invalid_argument("Wrong input, Expected eMessage");
    }
    return std::make_shared<ROSPublisherImpl<reswarm_msgs_matlab::ReswarmMsgMRPI,reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common>>();
  }
  std::shared_ptr<MATLABSubscriberInterface> 
         reswarm_msgs_matlab_ReswarmMsgMRPI_message::generateSubscriberInterface(ElementType type){
    if(type != eMessage){
        throw std::invalid_argument("Wrong input, Expected eMessage");
    }
    return std::make_shared<ROSSubscriberImpl<reswarm_msgs_matlab::ReswarmMsgMRPI,reswarm_msgs_matlab::ReswarmMsgMRPI::ConstPtr,reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common>>();
  }
#include "class_loader/register_macro.hpp"
// Register the component with class_loader.
// This acts as a sort of entry point, allowing the component to be discoverable when its library
// is being loaded into a running process.
CLASS_LOADER_REGISTER_CLASS(reswarm_msgs_matlab_msg_ReswarmMsgMRPI_common, MATLABROSMsgInterface<reswarm_msgs_matlab::ReswarmMsgMRPI>)
CLASS_LOADER_REGISTER_CLASS(reswarm_msgs_matlab_ReswarmMsgMRPI_message, ROSMsgElementInterfaceFactory)
#ifdef _MSC_VER
#pragma warning(pop)
#else
